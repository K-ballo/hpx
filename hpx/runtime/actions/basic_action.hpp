...
namespace hpx { namespace actions {
    ...
    ///////////////////////////////////////////////////////////////////////////
    template <auto F, typename Derived = detail::this_type>
    struct action;
    ...
    struct direct_action
      : action<F,
            typename detail::action_type<direct_action<F, Derived>,
                Derived>::type>
    {
        using derived_type =
...
#endif
    {
        using type = action<F, Derived>;
    };

    template <auto F, typename Derived>
    struct make_action<F, Derived, std::true_type>
#if defined(HPX_HAVE_ACTION_BASE_COMPATIBILITY)
      : direct_action<F, Derived>
#endif
    {
        using type = direct_action<F, Derived>;
    };

    template <auto F, typename Derived = detail::this_type>
    struct make_direct_action : make_action<F, Derived, std::true_type>
    {
    };

// Macros usable to refer to an action given the function to expose
#define HPX_MAKE_ACTION(func) hpx::actions::make_action<&func> /**/ /**/
#define HPX_MAKE_DIRECT_ACTION(func)                                           \
    hpx::actions::make_direct_action<&func> /**/ /**/
