...
namespace hpx { namespace performance_counters { namespace memory {
    void register_counter_types()
    {
        ...
        pc::install_counter_type("/runtime/memory/virtual",
            HPX_MONOSTATE_FUNCTION(&read_psm_virtual),
            "returns the amount of virtual memory currently allocated by the "
            "referenced locality",
            "bytes", pc::counter_raw);
        pc::install_counter_type("/runtime/memory/resident",
            HPX_MONOSTATE_FUNCTION(&read_psm_resident),
    ...
    defined(__linux__) || defined(HPX_WINDOWS)
        ...
        pc::install_counter_type("/runtime/memory/total",
            HPX_MONOSTATE_FUNCTION(&read_total_mem_avail),
        ...
        hpx::startup_function_type& startup_func, bool& pre_startup)
    {
        ...
        startup_func =
            HPX_MONOSTATE_FUNCTION(&register_counter_types);    // function to run during startup
        pre_startup =
