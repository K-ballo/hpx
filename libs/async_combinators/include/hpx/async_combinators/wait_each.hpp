...
namespace hpx { namespace lcos {
    ...
    {
        lcos::when_each(HPX_FWD(f), lazy_values).wait();
    }
    ...
    void wait_each(F&& f, Iterator begin, Iterator end)
    {
        lcos::when_each(HPX_FWD(f), begin, end).wait();
    }

    template <typename F, typename Iterator>
    void wait_each_n(F&& f, Iterator begin, std::size_t count)
    {
        when_each_n(HPX_FWD(f), begin, count).wait();
    }

    template <typename F>
    void wait_each(F&& f)
    {
        lcos::when_each(HPX_FWD(f)).wait();
    }
    ...
    wait_each(F&& f, Ts&&... ts)
    {
        lcos::when_each(HPX_FWD(f), HPX_FWD(ts)...).wait();
    }
}}    // namespace hpx::lcos
