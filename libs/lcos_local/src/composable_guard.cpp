...
namespace hpx { namespace lcos { namespace local {
    ...
    namespace detail {
        ...
        struct guard_task : detail::debug_object
        {
            ...
            guard_task()
              : next(nullptr)
              , run(HPX_MONOSTATE_FUNCTION(&nothing))
              , single_guard(true)
            {
            }
            guard_task(bool sg)
    ...
    static void stage_task(stage_data* sd, std::size_t i, std::size_t n)
    {
        ...
        else
        {
            ...
            stage->run = util::bind_front(HPX_MONOSTATE_FUNCTION(&stage_task), sd, k, n);
    ...
    void run_guarded(guard_set& guards, detail::guard_function task)
    {
        ...
        int k = 0;
        sd->stages[k]->run = util::bind_front(HPX_MONOSTATE_FUNCTION(&stage_task), sd, k, n);
        sd->gs = guards;
